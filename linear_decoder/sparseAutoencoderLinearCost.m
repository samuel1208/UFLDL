...
a_2 = sigmoid(z_2); 
rho_hat = sum(a_2, 2) / m; % Sparsity Parameter. 
                       
z_3 = W2 * a_2 + repmat(b2, 1, m);
a_3 = z_3;  %%% linear : difference to the sigmoid
diff = a_3 - data;

sparse_penalty = kl(sparsityParam, rho_hat);
J_simple = sum(sum(diff.^2)) / (2*m);
reg = sum(W1(:).^2) + sum(W2(:).^2);  %%%Weight decay, Don't include the bias
cost = J_simple + beta*sparse_penalty + lambda*reg/2;

%%% Back propagation
delta_3 = diff; %%% linear : difference to the sigmoid
